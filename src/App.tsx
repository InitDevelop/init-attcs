...
import LectureRaw from './util/LectureRaw';
import { convertToLecture } from './util/LectureUtil';
import { Dictionary, NumberPair } from './util/Util';
...
function App() {
  ...
  ****************************************************************************/
  ...
  const [lectureDatabase, setLectureDatabase] = useState<Lecture[]>(
    (lectureData as { subjects: LectureRaw[] }).subjects.map(raw => convertToLecture(raw))
    );
  ...
  ****************************************************************************/
  ...
  const [tooltipPosition, setTooltipPosition] = useState<NumberPair>({ x: 0, y: 0 });
  ...
  const isExistingSubj = (lecture: Lecture) => {
    ...
    for (let i = 0; i < selSubj.length; i++) {
      if (selSubj[i].subjectID === lecture.subjectID) {
        if (allowMult) {
          if (selSubj[i].lectureID === lecture.lectureID) {
            ret = true;
            break;
          }
        } else {
  ...
  useEffect(() => {
    setShownLectures(
      ...
      ).sort((a, b) => `${a.subjectID} ${a.lectureID}`.localeCompare(`${b.subjectID} ${b.lectureID}`))
  ...
  ****************************************************************************/
  ...
  const [customLectures, setCustomLectures] = useState<CustomLecture[]>([]);
  ...
  const [priority, setPriority] = useState<Dictionary<number>>({
    "empty": 1, "time": 5, "morning": 3, "count": 2, "lunch": 4, "space": 6
  });

  const addLectureToGroup = (lect: Lecture) => {
    const IDs = lectureGroups.map((lg: LectureGroup) => lg.subject.subjectID);
    let copy = lectureGroups;
    if (IDs.includes(lect.subjectID)) {
      const index = lectureGroups.findIndex((lg: LectureGroup) => lg.subject.subjectID === lect.subjectID);
      copy[index].lectures.push(lect);
    } else {
      copy.push(
        new LectureGroup([lect], true));
    }
  ...
  const removeLectureFromGroup = (lect: Lecture) => {
    ...
    if (copy[index].lectures.length === 0) {
      copy = copy.filter(lg => lg.subject.subjectID !== lect.subjectID);
    }
    setLectureGroups(copy);
  }
  ...
  const includesLecture = (lect: Lecture) => {
    ...
    if (IDs.includes(lect.subjectID)) {
      const index = lectureGroups.findIndex((lg: LectureGroup) => lg.subject.subjectID === lect.subjectID);
      return lectureGroups[index].lectures.filter(l => l.lectureID === lect.lectureID).length > 0;
    } else {
      return false;
    }
  }
  ...
  useEffect(() => {
    setLectureDatabase(((lectureData as { subjects: LectureRaw[] }).subjects).map(raw => convertToLecture(raw)).concat(
      customLectures.map(cl => cl.lecture)
    ));
  }, [customLectures]);
